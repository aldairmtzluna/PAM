<?php
    class ListaMinModel extends Mysql{
        ...
        public function selectMinutas(){
            $sql = "SELECT c.cargo_nom as unidad, m.minuta_id as id, m.minuta_titulo as titulo, m.minuta_status as estado, DATE_FORMAT(m.minuta_fecha,'%d/%m/%Y') as fecha
        FROM minutas as m 
        INNER JOIN cargos as c ON m.minuta_unidad_admin = c.cargo_id
        WHERE m.minuta_id NOT IN (SELECT MIN(minuta_id) FROM minutas)";
